...
public interface FoodItemScoreRepository extends JpaRepository<FoodItemScore, Long> {
    ...
    @Query("SELECT SUM(LENGTH(f.badIngredients) - LENGTH(REPLACE(f.badIngredients, ',', '')) + 1) " +
            "FROM FoodItemScore f " +
            "WHERE f.user.username = :username AND f.badIngredients IS NOT NULL")
